...
public:
	...
	string cacheName;
        double clMissRate;
        long int nlNumReadsNotFromPre; // next level number of reads not from prefetches
        long int nlNumReadMissFromPre; // next level number of read misses from prefetches
        long int nlNumWrites;
        long int nlNumWriteMiss;
        long int nlNumWriteBacks;
        long int nlNumPre;
        long int nlNumReads;
        long int nlNumReadMiss;
        double nlMissRate;
        long int totalMemTraffic;
...
/*** Stats ***/
	...
	numL2readMissNotFromPre = 0;
        clMissRate = 0; // Current level cache miss rate

        nlNumReadsNotFromPre = 0; // next level number of reads not from prefetches
        nlNumReadMissFromPre = 0; // next level number of read misses from prefetches
        nlNumWrites = 0;
        nlNumWriteMiss = 0;
        nlNumWriteBacks = 0;
        nlNumPre = 0;
...
void exp1_2() {
	...
	for(L1_SIZE = 1024; L1_SIZE <= 1024 * 1024 ; L1_SIZE*=2 ) { // CHANGE TERMINATING CONDITION
		L1_ASSOC[0] = 1;
		L1_ASSOC[1] = 2;
		L1_ASSOC[2] = 4;
		L1_ASSOC[3] = 8;
		L1_ASSOC[4] = L1_SIZE/BLOCKSIZE ;

		for(int i=0;i<=4;i++) { // CHANGE TERMINATING CONDITION

			cout << "===== Simulator configuration =====" << endl;
			cout << "BLOCKSIZE:\t" << BLOCKSIZE << endl;
			cout << "L1_SIZE:\t" << L1_SIZE << endl;
			cout << "L1_ASSOC:\t" << L1_ASSOC[i] << endl;
			...
			Cache l1(BLOCKSIZE,L1_ASSOC[i],L1_SIZE,L1_PREF_N,L1_PREF_M,NULL,"L1");			
			l1.run(trace_file);
//			l1.printContent();

			l1.printStats();

			cout << "Miss Rate: " << L1_SIZE << " " << L1_ASSOC[i] <<  " " << l1.clMissRate << endl;
...
void exp4() {
	...
	for(L1_SIZE = 1024; L1_SIZE <= 32 * 1024 ; L1_SIZE*=2 ) { 
		...
		for(BLOCKSIZE = 16 ;BLOCKSIZE <=128 ;BLOCKSIZE*=2) { 
			...
			Cache l1(BLOCKSIZE,L1_ASSOC,L1_SIZE,L1_PREF_N,L1_PREF_M,NULL,"L1");			
			l1.run(trace_file);
//			l1.printContent();

			l1.printStats();

			cout << "Miss Rate: " << BLOCKSIZE << " " << L1_SIZE << " " << L1_ASSOC <<  " " << l1.clMissRate << endl;
...
void exp3() {
	...
	for(L1_SIZE = 1024; L1_SIZE <= 256 * 1024 ; L1_SIZE*=2 ) { // CHANGE TERMINATING CONDITION
		L1_ASSOC[0] = 1;
		L1_ASSOC[1] = 2;
		L1_ASSOC[2] = 4;
		L1_ASSOC[3] = 8;
		...
		for(int i=0;i<=4;i++) { // CHANGE TERMINATING CONDITION
			...
			Cache l1(BLOCKSIZE,L1_ASSOC[i],L1_SIZE,L1_PREF_N,L1_PREF_M, &l2 ,"L1");			
			l1.run(trace_file);
//			l1.printContent();

			l1.printStats();

			cout << "Miss Rate: " << L1_SIZE << " " << L1_ASSOC[i] <<  " L1_Miss_rate " << l1.clMissRate << " L2_Miss_rate " << l1.nlMissRate << endl;
...
void exp6() {
	...
	for(BLOCKSIZE = 16 ; BLOCKSIZE <= 64 ; BLOCKSIZE*=2) {
		...
		for(L1_SIZE = 1024; L1_SIZE <= 16 * 1024 ; L1_SIZE*=2 ) { // CHANGE TERMINATING CONDITION
			for(L1_PREF_N = 1 ; L1_PREF_N <= 4 ; L1_PREF_N++) {
				for(L1_PREF_M = 1 ; L1_PREF_M <= 4 ; L1_PREF_M ++ ) {
